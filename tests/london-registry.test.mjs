...
test("sources/london.json contains exactly the 6 live-verified first-tranche entries plus the 2 live-verified second-tranche entries", async () => {
...
const SMALL_BESPOKE_SOURCE_IDS = new Set(["100-club-london", "the-underworld-london", "jazz-cafe-posk-london", "jamboree-london"]);
const MINOR_ADAPTER_SOURCE_IDS = new Set([...SQUARESPACE_SOURCE_IDS, "eventim-apollo-london"]);
const SECOND_TRANCHE_RESEARCH_IDS = new Set(["eventim-apollo-london", "jamboree-london"]);
...
test("every entry is TECHNICAL_PATH_PROVEN/TECHNICALLY_REVIEWED with acquisition_method STABLE_EVENT_PAGE (no source in this tranche relies on JSON-LD)", async () => {
  const registry = await loadJson("../sources/london.json");
  assert.equal(MINOR_ADAPTER_SOURCE_IDS.size + SMALL_BESPOKE_SOURCE_IDS.size, registry.entries.length);
  for (const entry of registry.entries) {
    ...
    assert.equal(
      entry.research_provenance.research_id,
      SECOND_TRANCHE_RESEARCH_IDS.has(entry.id) ? "BEATMAPPED-LONDON-SECOND-LIVE-TRANCHE-01" : "BEATMAPPED-LONDON-FIRST-TRANCHE-MAIN-REBASE-AND-MUSIC-GATE-01",
    );
    if (MINOR_ADAPTER_SOURCE_IDS.has(entry.id)) {
      ...
      assert.fail(`${entry.id}: unexpected source id, not in either known collector family list`);
    }
  }
});
