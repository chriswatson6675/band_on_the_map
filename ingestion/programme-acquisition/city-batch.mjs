...
export async function runCityAcquisition({ sources, fetchDocument, concurrency = 4, perHost = 1, detailLimit = 12 } = {}) {
  ...
  const results = await mapBounded(sources, async (source) => {
    ...
    if (!source.programme_url) return { ...base, state: "PROGRAMME_SOURCE_UNRESOLVED", residue: true, normalized_event_count: 0, proven_event_count: 0, evidence: [] };
    let programme;
    try { programme = await fetchDocument(source.programme_url); } catch (error) { return { ...base, state: "NETWORK_FAILURE", residue: true, error: String(error), normalized_event_count: 0, proven_event_count: 0, evidence: [] }; }
    const preliminary = routeProgrammeSource(programme);
    if (!preliminary.selected || preliminary.residue_state) {
      return { ...base, programme_url: programme.url, fingerprint: preliminary.fingerprint, candidate_routes: preliminary.routes, routes_attempted: preliminary.selected ? [preliminary.selected] : [], collector: preliminary.selected?.mechanism ?? null, state: preliminary.residue_state ?? "SOURCE_FINGERPRINT_UNSUPPORTED", residue: true, normalized_event_count: 0, proven_event_count: 0, observations: [], proofs: [], evidence: [programme], completed_at: new Date().toISOString() };
    }
    ...
    return { ...base, programme_url: programme.url, fingerprint: outcome.fingerprint, candidate_routes: outcome.routes, routes_attempted: outcome.selected ? [outcome.selected] : [], collector: outcome.selected?.mechanism ?? null, state: outcome.state, residue: outcome.residue, normalized_event_count: outcome.records?.length ?? 0, proven_event_count: outcome.observations.length, observations: outcome.observations, proofs: outcome.proofs, evidence: [programme, ...details], completed_at: new Date().toISOString() };
  }, { concurrency, perHost });
  return results;
}
