...
const input = JSON.parse(fs.readFileSync(new URL("./configuration-batch-01.json", import.meta.url)));
const rows = [];
...
fs.writeFileSync(new URL("./offline-proof-audit.json", import.meta.url), `${JSON.stringify({ artifact_type: "LONDON_CONFIGURATION_BATCH_01_OFFLINE_PROOF", input: "configuration-batch-01.json", rule: "Retained first-party detail canonical URL must equal retrieved document URL and supplied JSON-LD event URL where present.", rows, decision_counts }, null, 2)}\n`);
