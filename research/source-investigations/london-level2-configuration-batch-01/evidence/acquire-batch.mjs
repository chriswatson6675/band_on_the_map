...
const out = new URL("./configuration-batch-01.json", import.meta.url);
...
const candidates = triageFiles.flatMap((file) => JSON.parse(fs.readFileSync(new URL(file, import.meta.url))).results)
  .filter((candidate) => candidate.fit === "CONFIGURATION_ONLY" && candidate.programme_url && !candidate.blocked && !prior.has(candidate.candidate_id))
...
fs.writeFileSync(out, `${JSON.stringify({ artifact_type: "LONDON_CONFIGURATION_ONLY_LEVEL2_BATCH_01", capability_origin: "MIXED", request_bound: "25 deterministic candidates selected from retained Level-1 triage; one programme GET plus at most six same-origin generic detail GETs, no retry, browser, credential, or state change.", selected_count: candidates.length, results }, null, 2)}\n`);
